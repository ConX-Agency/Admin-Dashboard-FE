...
export const RegisterClientModal = ({
  ...
  const [industry, setIndustry] =
    useState<Client["industry"]>("Food & Beverage");
  const {
    control,
    handleSubmit,
    register,
  ...
  const onSubmit = async (data: Client) => {
    try {
      ...
      formData.append("company_name", formattedClient.company_name);
      formData.append(
        "person_in_charge_name",
        formattedClient.person_in_charge_name
      );
      formData.append(
        "person_in_charge_email",
        formattedClient.person_in_charge_email
      );
      formData.append("company_email", formattedClient.company_email);
      formData.append("contact_number", formattedClient.contact_number);
      formData.append(
        "alt_contact_number",
        formattedClient.alt_contact_number || ""
      );
      formData.append("industry", "Food & Beverage");
      formData.append("cuisine_type", formattedClient.cuisine_type);
      formData.append(
        "tnc_consent",
        JSON.stringify(formattedClient.tnc_consent)
      );
      formData.append("status", formattedClient.status);

      // Add addresses as JSON string
      formData.append("addresses", JSON.stringify(formattedClient.addresses));

      // Make API call
      console.log("this is form data", formattedClient.industry);
      const response = await axios.post(
        "https://backend-development-3158.up.railway.app/api/v1/clients",
        formData,
        {
          headers: {
            //Token should be changed after login function was created
            Authorization: `Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..80SFS0hOe1IF0RWGVk8cSTHKLAxBHq_NvsWrt6KKMWI`,
      ...
      handleRegister(formattedClient); // Keep this if needed for local state management
      closeRegisterModal();
      reset();
    } catch (error) {
          ...
          <form onSubmit={handleSubmit(onSubmit)}>
            <div className="grid xxxs:grid-cols-2 sm:grid-cols-4 lg:grid-cols-6 items-center gap-4 mb-4">
              <Input
                ...
                className={`xxxs:col-span-2 sm:col-span-4 lg:col-span-2 ${
              ...
              <Input
                ...
                {...register("contact_number", {
                  ...
                  pattern: {
                    value: /^\+\d{1,4}\d{7,15}$/,
                    message:
                      "Contact Number must include country code and be digits only.",
                  },
                  minLength: {
                    value: 8,
              ...
              <Input
                ...
                className={`xxxs:col-span-2 sm:col-span-4 lg:col-span-2 ${
                  errors.person_in_charge_name ? "border-red-500" : ""
                }`}
              ...
              <Input
                ...
                className={`col-span-2 ${
                  errors.person_in_charge_email ? "border-red-500" : ""
                }`}
                {...register("person_in_charge_email", {
                  required: {
                    value: true,
                    message: "Person in Charge's Email is required.",
                  },
                })}
              />
              <Input
                ...
                className={`col-span-2 ${
                  errors.alt_contact_number ? "border-red-500" : ""
                }`}
                {...register("alt_contact_number", {
                  ...
                  minLength: {
                    value: 8,
                    message:
                      "Alternative Contact Number must be at least 8 digits.",
                  },
                  maxLength: {
                    value: 19, // + (1-4 country code) + (7-15 phone number)
                    message:
                      "Alternative Contact Number must not exceed 19 digits.",
                  },
                })}
              />
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <Button
                    variant="ghost"
                    className="col-span-2 px-3 border justify-between w-full"
                  >
              ...
              <Input
                ...
                className={`col-span-2 ${
                  errors.cuisine_type ? "border-red-500" : ""
                }`}
                  ...
                  <div className="grid xxxs:grid-cols-4 sm:grid-cols-6 items-center gap-4">
                    <AddressDropdowns
                      ...
                      setCountry={(value: string) => {
                        setValue(`addresses.${index}.country`, value, {
                          shouldValidate: true,
                        });
                        trigger(); //retrigger validation after fixing error.
                      }}
                      countryMessage={`Address #${
                        index + 1
                      ...
                      setState={(value: string) => {
                        setValue(`addresses.${index}.state`, value, {
                          shouldValidate: true,
                        });
                        trigger();
                      }}
                      stateClassname={`col-span-2 ${
                        errors.addresses?.[index]?.state ? "border-red-500" : ""
                      }`}
                      ...
                      setCity={(value: string) => {
                        setValue(`addresses.${index}.city`, value, {
                          shouldValidate: true,
                        });
                        trigger();
                      }}
                      cityClassname={`col-span-2 ${
                        errors.addresses?.[index]?.city ? "border-red-500" : ""
                      }`}
                    ...
                    <Input
                      ...
                      {...register(`addresses.${index}.postcode` as const, {
                        required: {
                          ...
                          message: `Address #${
                            index + 1
                          }'s Postcode is required.`,
                        },
                        minLength: {
                          value: 4,
                          message: `Address #${
                            index + 1
                          }'s Postcode must be at least 4 numbers.`,
                        },
                        maxLength: {
                          value: 6,
                          message: `Address #${
                            index + 1
                          }'s Postcode must be no more than 6 numbers.`,
                        },
                        pattern: {
                          value: /^\d+$/,
                          message: `Address #${
                            index + 1
                          }'s Postcode must contain numbers only.`,
                        },
                      })}
                      className={`xxxs:col-span-2 col-span-1 ${
                    ...
                    <Input
                      ...
                      className={`xxxs:col-span-4 col-span-3 ${
            ...
            <div className="flex items-center space-x-2">
              ...
              >
                By submitting this form, I acknowledge and agree that my
                information will be processed in accordance with ConX Agency's
                Terms & Conditions.
